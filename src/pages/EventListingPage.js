...
export default function EventListingPage() {
  ...
  useEffect(() => {
    ...
    // Make an api request
    axios
      .get("http://localhost:8000/app/events/")
      .then((res) => {
        ...
        <div className="EventListingPageRight">
          ...
          <div className="EventListingCardBox">
            {eventsData.map((event) => {
              ...
              return (
                <Link
                  className="EventsOptions"
                  to={`/events/${event.id}/${event.name}/${event.type}/${event.address}/${event.min_audience}/${event.max_audience}/${event.start_date}/${event.end_date}/${event.start_time}/${event.end_time}/${event.description}/${imageUrl}/${plans}`}
                >
