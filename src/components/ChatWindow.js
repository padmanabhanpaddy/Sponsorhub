...
export default function ChatWindow(props) {
  ...
  useEffect(() => {
    ...
    axios.post("http://localhost:8000/app/get_user_sponsor_chats/", data)
    .then((response) => {
      ...
      chats.forEach(chat => {
        ...
        if((chat.sender === "sponsor" && !user_email) || 
            (chat.sender === "user" && user_email)){
          console.log("Sent by sponsor")
          setMessages(prevMessages => [
            ...prevMessages,
            { position: 'right', type: 'text', text: chat.message, date: new Date(), sender: chat.sender },
          ]);
        }
        else{
          setMessages(prevMessages => [
            ...prevMessages,
            { position: 'left', type: 'text', text: chat.message, date: new Date(), sender: chat.sender },
          ]);
        }
  ...
  useEffect(() => {
    const newSocket = new WebSocket(`ws://127.0.0.1:8000/chats/${props.userId}/${props.sponsorId}`);
    ...
    newSocket.onmessage = (event) => {
      ...
      if(message.event.sender === "sponsor" && user_email){
        ...
        setMessages(prevMessages => [
          ...prevMessages,
          { position: 'left', type: 'text', text: message.message, date: new Date(), sender: 'sponsor' },
        ]);
      }
      ...
      if(message.event.sender === "user" && !user_email){
        ...
        setMessages(prevMessages => [
          ...prevMessages,
          { position: 'left', type: 'text', text: message.message, date: new Date(), sender: 'user' },
        ]);
      }
  ...
  const handleSendMessage = () => {
    ...
    if (socket && newMessage.trim() !== '') {
      socket.send(JSON.stringify({ message: newMessage, type: user_email?"user":"sponsor", userId: userId, sponsorId:sponsorId }));
      setMessages(prevMessages => [
        ...prevMessages,
        { position: 'right', type: 'text', text: newMessage, date: new Date(), sender: 'user' },
      ]);
      setNewMessage('');
    }
