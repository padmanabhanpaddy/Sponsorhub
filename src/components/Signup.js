...
export default function Signup(props) {
  ...
  const login = async() =>{
    ...
    if(props.is_user){
      axios.post("http://localhost:8000/app/save_user/", {
      email: email,
      password: password,
      name: name,
      contact: contact,
      ...
      axios.post("http://localhost:8000/app/save_sponsor/", {
      email: email,
      password: password,
      name: name,
      type: type,
    ...
    <>
      <div className="outer-container">
        ...
        <div className="emaildes">
          ...
          {/* Signin Input */}
          <input type="text" name="name" id="emailbox" onChange={(event)=>{setName(event.target.value)}}/>
        </div>

        {!props.is_user?<div className="emaildes">
          <div className="email">Type *</div>
          {/* Signin Input */}
          <input type="text" name="type" id="emailbox" onChange={(event)=>{setType(event.target.value)}}/>
        </div>:<div></div>}

        <div className="emaildes">
          <div className="email">Contact *</div>
          {/* Signin Input */}
          <input type="number" name="contact" id="emailbox" onChange={(event)=>{setContact(event.target.value)}}/>
        </div>

        <div className="emaildes">
          <div className="email">Address *</div>
          {/* Signin Input */}
          <input type="text" name="address" id="emailbox" onChange={(event)=>{setAddress(event.target.value)}}/>
        </div>
        ...
        {!props.is_user?<div>
          ...
          {/* Signin Input */}
          <input type="file" name="profile" onChange={(event)=>{setProfile(event.target.files[0])}} accept="image/*" />
        </div>:<></>}
        ...
        {/* Signin Button */}
        <input type="button" value="Sign Up" id="sign" color="white" onClick={login}/>
        <div className="imp">
